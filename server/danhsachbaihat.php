<?php
	...
	if (isset($_POST['idalbum'])) {
		...
		$query = "SELECT * FROM Mp3baihat WHERE FIND_IN_SET('$idalbum', IDAlbum)";
	}

	if (isset($_POST['idtheloai'])) {
		$idtheloai = $_POST['idtheloai'];
		$query = "SELECT * FROM Mp3baihat WHERE FIND_IN_SET('$idtheloai', IdTheLoai)";
	}

	if (isset($_POST['idplaylist'])) {
		$idplaylist = $_POST['idplaylist'];
		$query = "SELECT * FROM Mp3baihat WHERE FIND_IN_SET('$idplaylist', IdPlayList)";
	}

	if (isset($_POST['idquangcao'])) {
		$idquangcao = $_POST['idquangcao'];
		$queryquangcao = "SELECT * FROM Mp3quangcao WHERE Id = '$idquangcao'";
		$dataquangcao = mysqli_query($con, $queryquangcao);
		$rowquangcao = mysqli_fetch_assoc($dataquangcao);
		$id = $rowquangcao['Idbaihat'];
		$query = "SELECT * FROM Mp3baihat WHERE IdBaiHat = '$id'";
	}
